...
fn find_valid_points_in_line(
    ...
    for x in 0..max_row {
        let numerator = dy * (x - p1.0);
        if numerator % dx != 0 {
            continue;
        }
        let y = p1.1 + numerator / dx;

        if is_valid(x, y, max_row, max_col) {
            res.push((x, y));
        }
    }
...
pub fn process(input: &str) -> i64 {
    ...
    antenna_map.values().for_each(|code| {
        code.iter().combinations(2).for_each(|points| {
            ...
            find_valid_points_in_line(points[0], points[1], curr_row, max_col)
                .iter()
                .for_each(|point| {
                    res_set.insert(*point);
                });
